...
class Registration(Resource):
	...
	def post(self):
		...
		user = db.session.execute(db.select(pengguna).filter_by(email=email)).first()
		if user:
			return {
				'message': 'This email address has been used!'
			}, 409 # HTTP Status Code for "Conflict".

		user 			= pengguna() # User instantiation.
		user.email 		= email
		user.name 		= name
...
class EditUser(Resource):
       ...
       def put(self):
        ...
        try:
            ...
            token = decodetoken(jwtToken)
            # print(token.get)
            user = pengguna.query.filter_by(email=token.get('email')).first()
            user.email = email
            user.name = name
...
class Password(Resource):
    ...
    def put(self):
        ...
        try:
            jwtToken    = bearerAuth[7:]
            token = decodetoken(jwtToken)
            user = pengguna.query.filter_by(id=token.get('user_id')).first()
...
class Detect(Resource):
    ...
    def post(self):
        ...
        if video and allowed_file(video.filename):
            filename = secure_filename(video.filename)
            video.save(os.path.join("./video/", filename))
            subprocess.run(['python', 'detect.py', '--source', f'./video/{filename}', '--weights', 'last.pt','--conf', '0.3', '--name', f'{filename}'])
            os.remove(f'./video/{filename}')
            print('success remove')
            return send_file(os.path.join(f"./runs/detect/{filename}", filename), mimetype='video/mp4', as_attachment=True, download_name=filename)
        else:
...
class Detect(Resource):
    ...
    def post(self):
        ...
        if image and allowed_file(image.filename):
            ...
            subprocess.run(['python', 'detect.py', '--source', f'./image/{filename}', '--weights', 'last.pt','--conf', '0.3', '--name', f'{filename}'])
...
def opencam():
    ...
    subprocess.run(['python', 'detect.py', '--source', '0', '--weights', 'last.pt','--conf', '0.3'])
    return "done"
